...
const fallbackStorePath = path.join(__dirname, 'recommendations.json');
...
function loadFallbackRecommendations() {
  try {
    if (fs.existsSync(fallbackStorePath)) {
      const fileContents = fs.readFileSync(fallbackStorePath, 'utf8').trim();
      if (!fileContents) {
        return [];
      }
    ...
    console.warn('폴백 저장소를 읽지 못했습니다.', error.message);
  }

  return [];
}

function saveFallbackRecommendations(items) {
  try {
    fs.mkdirSync(path.dirname(fallbackStorePath), { recursive: true });
    fs.writeFileSync(fallbackStorePath, JSON.stringify(items, null, 2));
  } catch (error) {
    console.warn('폴백 저장소를 저장하지 못했습니다.', error.message);
  }
}

function buildRecommendationRecord(payload, passwordHash) {
  return {
    id: Date.now() + Math.floor(Math.random() * 1000),
    menuName: String(payload.menuName || '').trim(),
    menuDesc: String(payload.menuDesc || '').trim(),
    menuReason: String(payload.menuReason || '').trim(),
    menuImage: String(payload.menuImage || '').trim(),
    weather: String(payload.weather || '').trim(),
    mood: String(payload.mood || '').trim(),
    type: String(payload.type || '').trim(),
    budget: String(payload.budget || '').trim(),
    note: String(payload.note || '').trim(),
    nickname: String(payload.nickname || '익명').trim(),
    passwordHash,
    createdAt: new Date().toISOString()
  };
}

function toRecommendationResponse(item) {
  return {
    id: item.id,
    menuName: item.menuName,
    menuDesc: item.menuDesc,
    menuReason: item.menuReason,
...
fallbackRecommendations = loadFallbackRecommendations();
...
async function ensureDatabaseReady(pool) {
  try {
    ...
    console.warn('데이터베이스 연결에 실패했습니다. 파일 기반 저장소를 사용합니다.', error.message);
    return false;
  }
}
      ...
      try {
        ...
        if (databaseAvailable) {
          try {
            ...
            databaseAvailable = false;
            console.warn('추천 목록 조회 실패, 폴백 저장소를 사용합니다.', error.message);
          }
        }

        const fallbackItems = fallbackRecommendations
          .slice()
          .reverse()
          .slice(0, limit)
          .map(item => toRecommendationResponse(item));
      ...
      try {
        ...
        if (databaseAvailable) {
          try {
            ...
            databaseAvailable = false;
            console.warn('추천 저장 실패, 폴백 저장소를 사용합니다.', error.message);
          }
        }

        const storedItem = buildRecommendationRecord({
          menuName,
          menuDesc,
          menuReason,
          menuImage: payload.menuImage || '',
        ...
        fallbackRecommendations = [storedItem, ...fallbackRecommendations];
        saveFallbackRecommendations(fallbackRecommendations);

        res.writeHead(201, { 'Content-Type': 'application/json; charset=utf-8' });
        res.end(JSON.stringify(toRecommendationResponse(storedItem)));
      } catch (error) {
        res.writeHead(400, { 'Content-Type': 'application/json; charset=utf-8' });
      ...
      try {
        ...
        if (databaseAvailable) {
          try {
            ...
            databaseAvailable = false;
            console.warn('추천 삭제 실패, 폴백 저장소를 사용합니다.', error.message);
          }
        }

        const beforeLength = fallbackRecommendations.length;
        fallbackRecommendations = fallbackRecommendations.filter(item => !(item.id === id && item.passwordHash === passwordHash));

        if (fallbackRecommendations.length === beforeLength) {
          res.writeHead(404, { 'Content-Type': 'application/json; charset=utf-8' });
          res.end(JSON.stringify({ error: '비밀번호가 맞지 않거나 이미 삭제된 글입니다.' }));
          return;
        }
