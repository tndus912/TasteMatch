...
function getPreferenceKey(prefs) {
  return `${prefs.weather}-${prefs.mood}-${prefs.type}-${prefs.budget}`;
}

function getSeenIndexesForPrefs(prefs) {
  const key = getPreferenceKey(prefs);
  if (!seenRecommendationsByKey[key]) {
    seenRecommendationsByKey[key] = [];
  }
  return seenRecommendationsByKey[key];
}

function rememberRecommendation(prefs, index) {
  const seenIndexes = getSeenIndexesForPrefs(prefs);
...
function recommendItem(prefs, excludedIndexes = []) {
  ...
  const excludedSet = new Set(
    Array.isArray(excludedIndexes) ? excludedIndexes : [excludedIndexes]
  );
  const seenIndexes = getSeenIndexesForPrefs(prefs);
  seenIndexes.forEach(index => excludedSet.add(index));

  scoredCandidates = scoredCandidates.filter(entry => !excludedSet.has(entry.originalIndex));
  ...
  rememberRecommendation(prefs, chosen.originalIndex);
...
async function fetchRecommendations() {
  try {
    ...
    if (!response.ok) {
      throw new Error('공유된 추천을 불러오지 못했습니다.');
    }
    return response.json();
  } catch (error) {
    communityFeed.innerHTML = `<div class="community-item">${error.message}</div>`;
    return [];
  }
}
...
function renderCommunityFeed(items) {
  if (!items.length) {
    communityFeed.innerHTML = '<div class="community-item">아직 공유된 추천이 없습니다. 첫 번째로 공유해보세요.</div>';
    return;
  }

  communityFeed.innerHTML = items.map(item => `
    <article class="community-item">
      <strong>${item.menuName}</strong>
      <div>${item.menuDesc}</div>
      <div class="community-meta-row">
        <span class="community-meta">${item.nickname} · ${new Date(item.createdAt).toLocaleString('ko-KR')}</span>
        <div class="community-actions">
          <input type="password" class="community-delete-input" placeholder="비밀번호" data-id="${item.id}" />
          <button type="button" class="community-delete-btn" data-id="${item.id}">삭제</button>
        </div>
      </div>
      ${item.note ? `<div class="community-meta">${item.note}</div>` : ''}
    </article>
  `).join('');

  communityFeed.querySelectorAll('.community-delete-btn').forEach(button => {
    button.addEventListener('click', () => handleDeleteRecommendation(button.dataset.id));
  });
}
...
async function handleDeleteRecommendation(id) {
  const input = communityFeed.querySelector(`.community-delete-input[data-id="${id}"]`);
  const password = input ? input.value.trim() : '';

  if (!password) {
    const enteredPassword = window.prompt('삭제할 비밀번호를 입력해 주세요.');
    if (!enteredPassword) {
      return;
    }
    if (input) {
...
function renderResult(result) {
  ...
  currentRecommendation = {
    menuName: item.name,
    menuDesc: item.desc,
    menuReason: item.reason,
    menuImage: item.image,
    weather: result.prefs.weather,
    mood: result.prefs.mood,
    type: result.prefs.type,
    budget: result.prefs.budget
  };
...
function handleFormSubmit(event) {
  ...
  currentPreferenceKey = getPreferenceKey(prefs);
  seenRecommendationsByKey[currentPreferenceKey] = [];
...
function handleReroll() {
  ...
  if (result.noAlternative) {
    alert('추천 결과가 없습니다. 메인 화면으로 돌아갑니다.');
    showHome();
    return;
  }
...
async function handleShareSubmit(event) {
  ...
  if (!currentRecommendation) {
    alert('먼저 추천 결과를 확인한 뒤 공유해 주세요.');
    return;
  }

  const nicknameInput = shareForm.querySelector('#share-nickname');
  const noteInput = shareForm.querySelector('#share-note');
  const passwordInput = shareForm.querySelector('#share-password');
  const payload = {
    ...currentRecommendation,
  ...
  try {
    ...
    const data = await response.json();
    if (!response.ok) {
      throw new Error(data.error || '공유하지 못했습니다.');
    }
...
refreshFeedBtn.addEventListener('click', refreshCommunityFeed);
refreshCommunityFeed();
